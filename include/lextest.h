...
namespace detail {
template <typename T> std::string to_string_any(T value) {
    if constexpr (std::is_same_v<std::remove_cv_t<std::remove_reference_t<T>>,
        ...
        return value ? "true" : "false"; // to_string bool'u "0"/"1" yapar
    } else {
...
#define __LX_TEST_EVENT_IMPL(a, b, OP, FILE_, LINE_)                           \
    [=, &self]() {                                                             \
        ...
        ev.assertion.expression = #a " " #OP " " #b;                           \
                                                                               \
        ev.assertion.result = (a OP b) ? ::lx::test_event_result::pass         \
                                       : ::lx::test_event_result::fail;        \
                                                                               \
        ev.assertion.evaluated = ::lx::detail::to_string_any(a) +              \
                                 " " #OP " " + ::lx::detail::to_string_any(b); \
                                                                               \
...
#define __LXSTREQ_IMPL(a, b, FILE_, LINE_)                                     \
    [=, &self]() {                                                             \
        auto &ev = self->events.emplace_back();                                \
                                                                               \
        ev.assertion.expression = #a " equals " #b;                            \
                                                                               \
        ev.assertion.result = (std::string(a) == std::string(b))               \
